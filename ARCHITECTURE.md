...
> **Status: Phases 1–12 complete.** The Requirement Analyzer (Phase 3), Task Planner / Engineer Review (Phase 4), task-level AI Assistance (Phase 5), controlled Artifact Generation (Phase 6), the Validation Engine (Phase 7), the mandatory URL Shortener use case (Phase 8), a brownfield performance optimization against it (Phase 9), an ambiguous-requirement scenario resolved by explicit engineer decision (Phase 10), a full end-to-end UI (Phase 11), and a security/performance hardening review (Phase 12) all exist — see the Components section below for what's real vs. still proposed. **AI never modifies the repository outside the sandboxed `generated/` workspace, executes commands, or applies its own output without an explicit engineer Accept/Reject in every phase implemented so far** — see [docs/api-design.md](docs/api-design.md) "No Automatic Code Execution" and "Controlled File Writes".
>
> **The URL shortener (Phase 8) is a demonstration use case built *through* this workbench** — its requirement, analysis, and task plan were processed by the same Requirement Analyzer / Task Decomposer above, its create-URL task was routed through AI-assist → engineer review → artifact generation → artifact review → validation (see `backend/tests/test_url_shortener_workbench_flow.py`, a permanent, re-runnable proof of that chain, and [docs/REQUIREMENT_TRACEABILITY.md](docs/REQUIREMENT_TRACEABILITY.md) for a real, captured chain of IDs) — it is not a disconnected application that happens to live in the same repository. See [docs/api-design-url-shortener.md](docs/api-design-url-shortener.md) for its own API surface, and `docs/adr/ADR-002` through `ADR-004` for its engineering decisions.
>
...
>
> **Phase 11 turns every component below into one coherent UI** (`frontend/src/components/AppShell.tsx` + 9 screens) demonstrating the full Requirement → Analysis → Plan → Tasks → AI Runs → Artifacts → Validation → Final Report chain, plus a Scenarios screen for Greenfield/Brownfield/Ambiguous — see [docs/DEMO_GUIDE.md](docs/DEMO_GUIDE.md) for a walkthrough.
>
> **Phase 12 reviewed the whole application for security and performance** (14 review areas — auth, injection classes, artifact/validation-command security, AI/secret handling, URL-shortener-specific risks, dependency scanning, performance) and closed a concurrency gap prior phases explicitly left `NOT VALIDATED` (a genuine 20-thread concurrent-load test against the redirect endpoint, 0 lost updates) — see [docs/security.md](docs/security.md) and [docs/performance.md](docs/performance.md).
...
* **Workbench UI** *(implemented, Phase 11)* — `frontend/src/components/AppShell.tsx` hosts navigation (Dashboard/Requirement/Engineering Plan/Tasks/AI Runs/Artifacts/Validation/Scenarios/Final Report) and a project selector over `GET /api/v1/requirements` (added Phase 11). `frontend/src/hooks/useProjectData.ts` assembles one consistent snapshot per selected requirement (requirement + analysis, plan + tasks + embedded AI runs, each task's artifacts, each artifact's validations) so every screen reads from the same data rather than re-fetching and potentially disagreeing. The Artifact screen maps the backend's real `PENDING_REVIEW`/`APPROVED`/`NEEDS_REVISION`/`REJECTED` enum to a display-only label folding in validation outcome — a deliberate choice to keep the backend enum authoritative rather than rename it for UI convenience (see `AI_USAGE.md` TASK-009).
...
Reviewed in full in Phase 12 — see [docs/security.md](docs/security.md) for the complete review. Summary of where trust boundaries actually sit:

* **Client → API:** every write endpoint validates its input at the Pydantic schema layer (length caps, scheme/host allowlists for URLs, closed enums for decision/validation types) before any service code runs.
* **API → Database:** all access goes through SQLAlchemy's ORM/Core query builder — no raw SQL string is ever constructed from request data.
* **API → Filesystem (artifact writes):** every write is contained to `generated/` by `app.utils.safe_path.resolve_artifact_path`, with two independent checks (explicit `..`/absolute-path rejection, then a resolved-path containment check) — see Controlled File Writes above.
* **API → Subprocess (validation runs):** `validation_type` is a closed enum; each maps to exactly one hardcoded, argument-list `subprocess.run` call. There is no code path from any request field into a shell command.
* **API → AI Provider:** untrusted content (requirement text, task descriptions, engineer instructions) is placed only in the *user* prompt turn, never the *system* prompt, and Anthropic's forced tool-call mechanism constrains responses to schema-shaped JSON. The deeper boundary is architectural: no AI output is ever auto-trusted — every recommendation, task, and artifact requires an explicit engineer decision before it has any effect, which bounds even a successful prompt-injection attempt to "produce a bad recommendation for a human to reject."
* **Secrets:** `AI_API_KEY` is environment-variable-only, never logged, never present in prompt content (verified by reading every prompt-construction call site).
* **Known, disclosed gaps (not fixed):** no authentication/authorization on any endpoint; no rate limiting on any endpoint. Both are appropriate-for-scope decisions for a local prototype, not oversights — see [docs/security.md](docs/security.md) Remaining Risks.
...
**Actual vs. proposed, stated plainly:** React/TypeScript, FastAPI, Pytest, OpenAPI, and the AI provider abstraction are implemented and in active use. PostgreSQL, Redis, and Docker remain proposed/present-but-unexercised — see [README.md](README.md) §5 for the exact status of each. Nothing above claims a technology is in production use when it has only been scaffolded.
